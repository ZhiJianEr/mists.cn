...
class td_smart_list_rd_6 extends td_smart_list {
    ...
    protected function render_before_list_wrap() {
        ...
        //wrapper with id for smart list wrapper type red 6
        $buffy .= '<div class="td_smart_list_rd_6' . $td_class_nr_of_columns . '">';
    ...
    protected function render_list_item($item_array, $current_item_id, $current_item_number, $total_items_number) {
        ...
        //get the title
        $smart_list_rd_6_title = '';
        if(!empty($item_array['title'])) {
            $smart_list_rd_6_title = $item_array['title'];
        }

        //creating each slide
        $buffy .= '<div class="td-item">';
        $buffy .= '<div class="td-number-and-title"><h2 class="td-title-smart"><span class="td-sml-current-item-nr">' . $current_item_number. '</span><span class="td-sml-current-item-title">' . $smart_list_rd_6_title . '</span></h2></div>';
        ...
        if(td_global::$cur_single_template_sidebar_pos == 'no_sidebar') {
            ...
            $first_img_info = wp_get_attachment_image_src($item_array['first_img_id'], 'td_610x0');
        }
        if (!empty($first_img_info[0])) {
            ...
            $buffy .= '
                <div class="td-sml-figure">
                    <figure class="td-slide-smart-list-figure td-slide-smart-list-rd-6' . $smart_list_lightbox . '">
        ...
        // ad smart list
        $tds_smart_list_title = td_util::get_option('tds_smart_list_rd_6_title'); // ad title
        $buffy .= td_global_blocks::get_instance('td_block_ad_box')->render(array('spot_id' => 'smart_list_rd_6', 'spot_title' => $tds_smart_list_title));
    ...
    protected function render_after_list_wrap() {
        $buffy = '';
        $buffy .= '</div>'; // /.td_smart_list_rd_6  wrapper with id
