 ...
 */
class td_block_big_grid_rd_bg_13 extends td_block {
    ...
    function inner($posts, $td_column_number = '') {
        ...
        if (!empty($posts)) {
            ...
            if ($td_column_number==1 || $td_column_number==2) {
                $buffy .= td_util::get_block_error('Big grid red 13', 'Please move this shortcode on a full row in order for it to work.');
            } else {
                $buffy .= '<div class="td-big-grid-wrapper td-posts-' . $td_count_posts . '">';

                $post_count = 0;

                // when 1 posts make post scroll full
                $td_scroll_posts = '';
                if (count($posts) == 2) {
